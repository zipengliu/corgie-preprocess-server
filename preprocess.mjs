...
import forceBundling from "./forceBundling.mjs";
import mingleBundling from "./mingleBundling.mjs";
...
function getFeatureDistMax(n, features, distMatFeature) {
    let s = 0;
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            const dFeat = getEuclideanDistance(features[i], features[j]);
    ...
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            if (distMatFeature[i].hasOwnProperty(j)) {
                ...
                distMatFeature[j][i] = distMatFeature[i][j];
            }
        }
    }
    return s;
...
function computeDistances(
    ...
    let binsFeature = null,
        gridsFeature = null;
    if (features) {
        console.assert(extent(distFeature)[1] <= 1.01, distFeature);
...
function csvPromise(filepath, options, isNumMatrix = false) {
...
function getUMAPresults(filepath, emb) {
    return csvPromise(filepath, { headers: false }, true).catch(() => {
...
async function mainProcess(dataDir, hops, graphMeta = {}) {
    console.log("==============");
    console.log("Pre-pocessing dataset ", dataDir, hops);

    // const graph = require(`${dataDir}/graph.json`);
    const graph = JSON.parse(await readFile(`${dataDir}/graph.json`));
    ...
    const layout = computeForceLayoutWithD3(graph.nodes.length, graph.edges, padding, false);

    layout.coords = layout.coords.map((c) => ({ x: c.x.toFixed(2), y: c.y.toFixed(2) }));
    const layoutPromise = writeFile(`${dataDir}/initial-layout.json`, JSON.stringify(layout)).then(() => {
        console.log("Writing intial layout successfully!");
    });
    compPromises.push(layoutPromise);

    // read the embeddings
    const emb = await csvPromise(`${dataDir}/node-embeddings.csv`, { headers: false }, true);

    // Compute UMAP
    const drPromise = getUMAPresults(`${dataDir}/umap.csv`, emb).then((umapRes) => {
        ...
        const drWritePromise = writeFile(
            `${dataDir}/umap.json`,
            JSON.stringify({ ...e, neighborBinning: b })
        ).then(() => {
            console.log("Writing UMAP results successfully!");
        });
        writePromises.push(drWritePromise);
    });
    compPromises.push(drPromise);

    // Read the features
    let attrs = null,
        denseFeatures = null;
    try {
        attrs = JSON.parse(await readFile(`${dataDir}/attr-meta.json`));
        denseFeatures = normalizeFeatures(attrs, graph.nodes);
    } catch (e) {
        console.log("No dense features");
    }
    let sparseFeatures = await csvPromise(`${dataDir}/features.csv`, { headers: false }, true).catch(
        () => {}
    );
    let features = sparseFeatures ? sparseFeatures : denseFeatures;
    ...
    if (features) {
        featureDistMax = getFeatureDistMax(numNodes, features, distMatFeature);
    ...
    const distPromise = writeFile(`${dataDir}/distances.json`, distJson).then(() => {
        console.log("Writing distance data successfully!");
    });
    compPromises.push(distPromise);
    ...
    const graphJson = JSON.stringify({
        ...
        sparseFeatures,
        denseFeatures,
        neighborMasks: graph.neighborMasks.map((m) => m.toString(16)),
        neighborMasksByHop: graph.neighborMasksByHop.map((h) => h.map((m) => m.toString(16))),
    });
    const graphDumpPromise = writeFile(`${dataDir}/graph.json`, graphJson).then(() => {
        console.log("Writing graph data successfully");
    });
    writePromises.push(graphDumpPromise);

    console.log("#Promises = ", compPromises.length);
    await Promise.all(compPromises);
    await Promise.all(writePromises);
    console.log("Finished... Update status to Ready");
    console.log("=======");
    writeFile(`${dataDir}/status`, "1");
}

export default mainProcess;
// if (require.main === module) {
//     const dataDir = process.argv[2];
//     const hops = process.argv[3];
//     mainProcess(dataDir, hops);
// }
